...
export type ChoreStatus = "unapproved" | "unclaimed" | "claimed" | "complete" | "disputed";
...
// RecentActivity interface removed - now using Chore type
// Mock data for homes
const HOMES: Home[] = [
 {
   id: "home_1",
...
const DISPUTES: Dispute[] = [
 {
   ...
   disputerName: "John",
   reason: "Not cleaned properly - dishes still dirty",
   status: "pending",
 ...
 },
 {
   ...
   disputerName: "Sarah",
   reason: "Incomplete dusting - missed the top shelves",
   status: "pending",
 ...
 },
 {
   ...
   choreId: "dispute-3",
   choreName: "Vacuuming",
   choreDescription: "Vacuum all carpets and floors",
   choreIcon: "zap",
   disputerEmail: "mike@example.com",
   disputerName: "Mike",
   reason: "Left vacuum cleaner out and didn't empty the bag",
   status: "pending",
   createdAt: "2024-01-13T12:20:00Z",
 },
];
// Recent activities are now generated from completed chores
...
export const getRecentActivitiesAPI = (homeID: string): Chore[] => {
 const threeDaysAgo = new Date();
 threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);
  return CHORES.filter((chore) =>
   chore.status === "complete" &&
   chore.homeID === homeID &&
   new Date(chore.time) >= threeDaysAgo
 );
};
export const createDisputeAPI = (
 ...
 if (!chore) {
  ...
  // Mark the chore as disputed so it won't appear in recent activities
